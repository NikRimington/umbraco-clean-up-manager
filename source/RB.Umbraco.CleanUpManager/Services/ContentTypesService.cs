...
namespace RB.Umbraco.CleanUpManager.Services
{
	...
	public class ContentTypesService : IContentTypesService
	{
		...
		public virtual List<IContentType> GetOrphanContentTypes()
		{
			try
			{
				var contentTypes = _contentTypeService.ContentTypeService.GetAllContentTypes().ToList();
		...
		/// <summary>
		/// Determines whether [is not part of another content type composition] [the specified content type].
		/// </summary>
		/// <param name="contentType">Type of the content.</param>
		/// <returns><c>true</c> if [is not part of another content type composition] [the specified content type]; otherwise, <c>false</c>.</returns>
		private bool CheckIfContentTypeIsNotPartOfAnotherContentTypeComposition(List<IContentType> results , IContentType contentType)
		{
		...
		private bool IsNotAParentOfAnotherContentType(IContentType contentType)
		{
			return !_contentTypeService.ContentTypeService.GetContentTypeChildren(contentType.Id).Any();
		}
		...
		private bool HasNoInstanceOfItsOwn(IContentType contentType)
		{
			return !_contentService.ContentService.GetContentOfContentType(contentType.Id).Any();
		}
		...
		public virtual bool DeleteOrphanContentTypes()
		{
			try
			{
				...
				{
					_contentTypeService.ContentTypeService.Delete(orphanContentType);
				}
		...
		protected internal virtual void LogCleanseOperation(List<IContentType> contentTypes)
		{
